rm(list=ls())
dev.off()
require (dendextend) #install.packages('dendextend') #easy visualization manipulation and comparison of dendrograms 
require(circlize) #install.packages('circlize')   #circle dendograms

class  = as.matrix(read.csv('../Data/ziplabel.csv'))
class=as.factor(class)
X      = read.csv('../Data/zipdata.csv')
N = dim(X)[1]
p = dim(X)[2]
...
class[grp == 1]
rect.hclust(as.hclust(hc), k = K, border = rainbow(10))
...
dend <- color_branches(dend, k=K,col=rainbow(10)) #, groupLabels=

# Manually match the labels with colors
labels_colors(dend) <-
  rainbow(10)[sort_levels_values(
    as.numeric(class)[order.dendrogram(dend)]
  )]
...
par(mar = c(3,3,3,7))
plot(dend, 
     main = "Clustered ZIP data set", 
     horiz =  TRUE,  nodePar = list(cex = .007))
numbers <- rev(levels(class))
legend("topleft",legend =paste("cluster", numbers), fill = rev(rainbow(10)))
