...
class  = csvread('ziplabel.csv');
X      = csvread('zipdata.csv');
...
for K = Kvec
    ...
    W(count) = 0; % Initialization
    for ik = 1:K 
                % <------------ Missing code, calculate W(count)
    ...
    for j=1:Nsim
        ...
        % simulated data).
        Wu(count,j) = 0; % Initialization
        for ik = 1:K     
               % <------------ Missing code, calculate Wu(count,j)
